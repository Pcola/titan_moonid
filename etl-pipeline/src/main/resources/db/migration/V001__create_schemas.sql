...
-- -------------------------------------------------------------
CREATE TABLE IF NOT EXISTS staging.humed_raw
(
...
CREATE INDEX IF NOT EXISTS idx_humed_raw_title_gin ON staging.humed_raw
    USING GIN (to_tsvector('simple', title));
CREATE INDEX IF NOT EXISTS idx_humed_raw_categories_gin ON staging.humed_raw
    USING GIN (categories);
CREATE INDEX IF NOT EXISTS idx_humed_raw_excluded ON staging.humed_raw(is_excluded)
    WHERE is_excluded = FALSE;

-- -------------------------------------------------------------
-- HUMED: Sync log
-- -------------------------------------------------------------
CREATE TABLE IF NOT EXISTS staging.humed_sync_log
(
    id                 SERIAL PRIMARY KEY,
...
-- -------------------------------------------------------------
CREATE TABLE IF NOT EXISTS staging.corwell_raw
(
    id                SERIAL PRIMARY KEY,
...
-- CORWELL: Cenník
-- -------------------------------------------------------------
CREATE TABLE IF NOT EXISTS staging.corwell_prices
(
    id         SERIAL PRIMARY KEY,
    cikkid     INTEGER NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_corwell_prices_cikkid ON staging.corwell_prices(cikkid);

-- -------------------------------------------------------------
-- CORWELL: Zásoby
-- -------------------------------------------------------------
CREATE TABLE IF NOT EXISTS staging.corwell_stock
(
    id         SERIAL PRIMARY KEY,
    cikkid     INTEGER     NOT NULL,
    cikkszam   VARCHAR(50) NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_corwell_stock_lookup ON staging.corwell_stock(cikkid, checked_at DESC);

-- -------------------------------------------------------------
-- CORWELL: Sync log
-- -------------------------------------------------------------
CREATE TABLE IF NOT EXISTS staging.corwell_sync_log
(
    id                SERIAL PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_categories_path ON catalog.categories USING GIST(path);
CREATE INDEX IF NOT EXISTS idx_categories_parent ON catalog.categories(parent_id);
CREATE INDEX IF NOT EXISTS idx_categories_active ON catalog.categories(is_active) WHERE is_active = TRUE;

-- -------------------------------------------------------------
-- CATEGORY_RULES: Pravidlá pre automatické mapovanie
-- -------------------------------------------------------------
CREATE TABLE IF NOT EXISTS catalog.category_rules
(
    id                      SERIAL PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_category_rules_lookup ON catalog.category_rules(source, is_active, priority)
    WHERE is_active = TRUE;

-- -------------------------------------------------------------
-- CATEGORY_EXCLUSIONS: Zdrojové kategórie na vylúčenie
-- -------------------------------------------------------------
CREATE TABLE IF NOT EXISTS catalog.category_exclusions
(
    id                      SERIAL PRIMARY KEY,
...
-- -------------------------------------------------------------
CREATE TABLE IF NOT EXISTS catalog.category_mapping_log
(
    id                  SERIAL PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_mapping_log_unmapped ON catalog.category_mapping_log(match_type)
...
CREATE INDEX IF NOT EXISTS idx_products_active ON catalog.products(is_active) WHERE is_active = TRUE;
CREATE INDEX IF NOT EXISTS idx_products_name_gin ON catalog.products USING GIN (to_tsvector('simple', name));

-- -------------------------------------------------------------
-- PRODUCT_SOURCES: Prepojenie na zdrojové feedy
-- -------------------------------------------------------------
CREATE TABLE IF NOT EXISTS catalog.product_sources
(
    id                    SERIAL PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_product_sources_product ON catalog.product_sources(product_id);
...
-- -------------------------------------------------------------
CREATE TABLE IF NOT EXISTS export.woocommerce_categories
(
    id             SERIAL PRIMARY KEY,
...
-- -------------------------------------------------------------
CREATE TABLE IF NOT EXISTS export.woocommerce_products
(
    id                SERIAL PRIMARY KEY,
...
-- -------------------------------------------------------------
CREATE TABLE IF NOT EXISTS export.woocommerce_sync_log
(
    id                SERIAL PRIMARY KEY,
...
CREATE
OR REPLACE FUNCTION catalog.update_category_path()
RETURNS TRIGGER AS $$
BEGIN
    IF
...
CREATE TRIGGER trg_woo_products_updated
    BEFORE UPDATE
    ON export.woocommerce_products
    FOR EACH ROW EXECUTE FUNCTION update_updated_at();
